...
import {
  ...
  Shuffle, Repeat, Repeat1, ArrowRight, Loader2, Plus, X, UploadCloud, Image as ImageIcon, Mic2, FolderPlus, Trash2, Clock, Home, ListMusic, LogOut, ChevronDown, RefreshCw, ListPlus, Moon, SlidersHorizontal, ArrowUpDown, Search
} from "lucide-react";
...
export default function App() {
  ...
  // SORT & SEARCH STATE (Independent per playlist ID)
  ...
  // VIEWED LIST DERIVATION WITH SEARCH & SORT
          ...
          {upcomingList.length > 0 ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {upcomingList.map((song, uIdx) => {
                ...
                return (
                  <div key={`next-${song.id}-${uIdx}`} onClick={() => {
                      setQueueCurrentTrack(null);
              ...
              <div style={{ width: "100%" }}>
                {playlistSongs.length > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "24px", justifyContent: isDesktop ? "flex-start" : "center", flexWrap: "wrap" }}>
                    {displayedSongs.length > 0 && (
                      <button onClick={() => handlePlaySong(0, displayedSongs, activePlaylistObj?.name || "Playlist")} className="hover-effect" style={{ width: "64px", height: "64px", borderRadius: "50%", background: COLORS.primary, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 8px 16px rgba(26,43,76,0.2)", flexShrink: 0 }}>
                        <Play size={28} fill={COLORS.bgPanel} color={COLORS.bgPanel} style={{ marginLeft: "4px" }} />
                      </button>
                    )}
                    
                    <div style={{ display: "flex", alignItems: "center", background: "rgba(26, 43, 76, 0.05)", borderRadius: "20px", padding: "6px 12px", border: `1px solid ${COLORS.border}`, flexGrow: isDesktop ? 0 : 1 }}>
                      ...
                      <input 
                        ...
                        style={{ background: "transparent", border: "none", outline: "none", marginLeft: "8px", fontSize: "14px", color: COLORS.primary, width: isDesktop ? "200px" : "100%" }}
                      />
                      {searchQuery && (
                        <X size={14} color={COLORS.textMuted} style={{ cursor: "pointer" }} onClick={() => setSearchQuery("")} />
                      )}
                    </div>
                ...
                {displayedSongs.length > 0 ? (
                  ...
                  <div style={{ textAlign: "center", padding: "80px 0", color: COLORS.textMuted }}>
                    {searchQuery ? (
                      <p style={{ margin: 0, fontSize: "16px" }}>No results found for "{searchQuery}".</p>
                    ) : (
