...
import {
  ...
  Shuffle, Repeat, Repeat1, ArrowRight, Loader2, Plus, X, UploadCloud, Image as ImageIcon, Mic2, ChevronDown, FolderPlus, Trash2, Clock, Home, Library, ListMusic, LogOut
} from "lucide-react";
...
export default function App() {
  ...
  // Helper to determine view modes
  const isDesktop = deviceType === "windows" || deviceType === "mac" || deviceType === "desktop";

  useEffect(() => {
    // Basic resize listener for dynamic responsiveness (optional enhancement)
    const handleResize = () => {
      if (window.innerWidth <= 768) setDeviceType("android"); // Force mobile view
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", fontFamily: "system-ui, -apple-system, sans-serif", background: "#000000", color: "#ffffff", display: "flex", flexDirection: "column", overflow: "hidden", boxSizing: "border-box" }}>
      ...
      <div style={{ height: "64px", flexShrink: 0, background: "#000000", display: "flex", justifyContent: "space-between", alignItems: "center", padding: isDesktop ? "0 24px" : "0 12px", borderBottom: "1px solid #1a1a1a", zIndex: 10 }}>
        <h1 style={{ margin: 0, fontSize: isDesktop ? "22px" : "18px", color: "#1DB954", fontWeight: "bold", letterSpacing: "-0.5px" }}>Euphony</h1>
        <div style={{ display: "flex", gap: isDesktop ? "12px" : "8px", alignItems: "center" }}>
          <button className="hover-effect" onClick={() => setShowUploadModal(true)} style={{ background: "#282828", border: "none", borderRadius: "20px", padding: isDesktop ? "8px 16px" : "8px 12px", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: "bold", whiteSpace: "nowrap" }}>
            <Plus size={16} color="#1DB954" /> {isDesktop && "Add Globally"}
          </button>
          <button className="hover-effect" onClick={() => setShowPlaylistModal(true)} style={{ background: "#282828", border: "none", borderRadius: "20px", padding: isDesktop ? "8px 16px" : "8px 12px", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: "bold", whiteSpace: "nowrap" }}>
            <FolderPlus size={16} color="#1DB954" /> {isDesktop && "New Playlist"}
          </button>
          <button className="hover-effect" onClick={() => supabase.auth.signOut()} style={{ background: "transparent", border: "1px solid #444", borderRadius: "20px", padding: isDesktop ? "8px 16px" : "8px", color: "#fff", cursor: "pointer", fontSize: "13px", fontWeight: "bold", display: "flex", alignItems: "center", gap: "4px" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden", padding: "8px", gap: "8px" }}>
        ...
        {isDesktop && (
          <div style={{ width: "240px", flexShrink: 0, background: "#121212", borderRadius: "8px", padding: "20px", display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", overflowY: "auto", flex: 1 }} className="custom-scrollbar">
              ...
              {userPlaylists.map(pl => (
                <div key={pl.id} onClick={() => { setSelectedPlaylistId(pl.id); setCurrentTrackIndex(0); }} className="sidebar-item" style={{ display: "flex", alignItems: "center", gap: "10px", color: selectedPlaylistId === pl.id ? "#ffffff" : "#b3b3b3", fontSize: "14px", padding: "4px 0" }}>
                  <ListMusic size={18} color={selectedPlaylistId === pl.id ? "#1DB954" : "#b3b3b3"} /> 
        ...
        <div style={{ flex: 1, minWidth: 0, background: "#121212", borderRadius: "8px", padding: isDesktop ? "32px" : "16px", overflowY: "auto", display: "flex", flexDirection: "column", paddingBottom: !isDesktop && currentTrack ? "80px" : "32px" }} className="custom-scrollbar">
          ...
          {selectedPlaylistId !== null && activePlaylistObj && (
            <div style={{ background: "linear-gradient(180deg, #2b3833 0%, #121212 100%)", padding: isDesktop ? "32px" : "20px", borderRadius: "12px", marginBottom: "24px", display: "flex", alignItems: isDesktop ? "flex-end" : "center", flexDirection: isDesktop ? "row" : "column", gap: "24px" }}>
              <div style={{ width: isDesktop ? "160px" : "140px", height: isDesktop ? "160px" : "140px", backgroundColor: "#282828", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 8px 24px rgba(0,0,0,0.6)", flexShrink: 0 }}>
                <FolderPlus size={isDesktop ? 64 : 48} color="#1DB954" />
              </div>
              <div style={{ flex: 1, minWidth: 0, textAlign: isDesktop ? "left" : "center" }}>
                ...
                <h2 style={{ margin: "4px 0 12px 0", fontSize: isDesktop ? "42px" : "28px", fontWeight: "bold", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{activePlaylistObj.name}</h2>
          ...
          {selectedPlaylistId !== null ? (
            <div style={{ width: "100%" }}>
              {playlistSongs.length > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "20px", justifyContent: isDesktop ? "flex-start" : "center" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isDesktop ? "40px 2fr 1.5fr 1.2fr 50px" : "40px 1fr 40px", padding: "0 12px 12px 12px", borderBottom: "1px solid #282828", color: "#b3b3b3", fontSize: "13px", fontWeight: "bold" }}>
                <span>#</span>
                <span>Title</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginTop: "8px" }}>
                {playlistSongs.length > 0 ? (
                  playlistSongs.map((track, index) => {
                    ...
                    return (
                      <div key={track.id} className="playlist-row" onClick={() => { setCurrentTrackIndex(index); setIsPlaying(true); }} style={{ display: "grid", gridTemplateColumns: isDesktop ? "40px 2fr 1.5fr 1.2fr 50px" : "40px 1fr 40px", alignItems: "center", padding: "10px 12px", borderRadius: "6px", cursor: "pointer" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
                          ...
                          <div style={{ minWidth: 0, flex: 1 }}>
                            <div style={{ fontSize: "14px", fontWeight: isSelected ? "bold" : "normal", color: isSelected ? "#1DB954" : "#ffffff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{track.title}</div>
                            <div style={{ fontSize: "12px", color: "#a0a0a0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{track.artist}</div>
                          </div>
                        </div>

                        {isDesktop && <span style={{ color: "#b3b3b3", fontSize: "14px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", paddingRight: "8px" }}>{track.album || "—"}</span>}
                        {isDesktop && <span style={{ color: "#b3b3b3", fontSize: "13px", paddingRight: "8px" }}>{formatDate(track.added_at)}</span>}
            ...
            /* GLOBAL LIBRARY */
            <div>
              <h2 style={{ fontSize: isDesktop ? "24px" : "20px", fontWeight: "bold", marginBottom: "20px", textAlign: isDesktop ? "left" : "center" }}>Global Library ({playlist.length})</h2>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {playlist.length > 0 ? (
                  playlist.map((track, index) => {
                    const isSelected = index === currentTrackIndex;
                    return (
                      <div key={track.id} className="playlist-row" onClick={() => { setCurrentTrackIndex(index); setIsPlaying(true); }} style={{ padding: "10px 12px", borderRadius: "8px", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", gap: isDesktop ? "16px" : "12px" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 }}>
                          {userPlaylists.length > 0 && (
                            <select 
                              ...
                              defaultValue=""
                              style={{ background: "#222", color: "#bbb", border: "1px solid #444", borderRadius: "6px", padding: "6px 8px", fontSize: "12px", cursor: "pointer", maxWidth: isDesktop ? "150px" : "120px", textOverflow: "ellipsis" }}
                            >
                  ...
                  <div style={{ textAlign: "center", padding: "80px 0", color: "#a0a0a0" }}>
                    ...
                    <p style={{ margin: 0, fontSize: "16px" }}>Your library is empty. Click "Add Globally" to upload tracks.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ width: "320px", flexShrink: 0, background: "#121212", borderRadius: "8px", padding: "24px", display: "flex", flexDirection: "column", boxSizing: "border-box", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
              ...
              <div style={{ width: "100%", marginBottom: "20px", flexShrink: 0 }}>
                <div style={{ width: "100%", aspectRatio: "1/1", borderRadius: "8px", overflow: "hidden", backgroundColor: "rgba(40,40,40,0.5)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 12px 30px rgba(0,0,0,0.6)", position: "relative" }}>
                  {showLyrics ? (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <div style={{ minWidth: 0, flex: 1, textShadow: "0 2px 8px rgba(0,0,0,0.8)" }}>
                  <h3 style={{ margin: "0 0 4px 0", fontSize: "18px", fontWeight: "bold", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentTrack.title}</h3>
                  <p style={{ margin: 0, color: "#b3b3b3", fontSize: "14px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentTrack.artist}</p>
                </div>
                <button onClick={() => setShowLyrics(!showLyrics)} style={{ background: showLyrics ? "#1DB954" : "rgba(255,255,255,0.1)", color: showLyrics ? "#000" : "#fff", border: "none", borderRadius: "20px", padding: "6px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", fontWeight: "bold", flexShrink: 0, marginLeft: "12px" }}>
              ...
              <div style={{ marginTop: "auto", paddingBottom: "8px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
                  <span style={{ fontSize: "11px", color: "#b3b3b3", minWidth: "32px" }}>{formatTime(currentTime)}</span>
                  ...
                  <span style={{ fontSize: "11px", color: "#b3b3b3", minWidth: "32px", textAlign: "right" }}>{formatTime(duration)}</span>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                    <button onClick={handlePrev} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", display: "flex" }}><SkipBack size={22} fill="currentColor" /></button>
                    ...
                    <button onClick={handleNext} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", display: "flex" }}><SkipForward size={22} fill="currentColor" /></button>
                  </div>
      ...
      {!isDesktop && currentTrack && (
        <div style={{ position: "fixed", bottom: "12px", left: "12px", right: "12px", background: "#282828", borderRadius: "8px", padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 8px 24px rgba(0,0,0,0.8)", zIndex: 2000, border: "1px solid #333" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", overflow: "hidden", flex: 1, minWidth: 0 }}>
            ...
            <div style={{ overflow: "hidden", flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: "12px", color: "#a0a0a0", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{currentTrack.artist}</div>
            </div>
          </div>
